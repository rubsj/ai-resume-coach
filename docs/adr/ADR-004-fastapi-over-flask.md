...
The 346-line `api.py` serves 9 endpoints using all 14 Pydantic schemas from `schemas.py` with zero adapter code — every schema is both the FastAPI parameter type and the OpenAPI documentation source. Swagger UI at `/docs` renders all endpoints interactively the moment the server starts, letting recruiters send real requests to `/review-resume` or `/search/similar-candidates` from the browser with no additional setup. Because FastAPI's TestClient wraps httpx, all 26 endpoint tests run without starting a server process — same pattern as Spring's MockMvc but with zero configuration — contributing to the 532-test suite that runs in ~2 seconds. Adding the `/feedback` endpoint (POST with `FeedbackRequest` body, returns `FeedbackResponse`) took 15 lines of code including file I/O, because the framework handled parsing, validation, serialization, and documentation automatically. Structured 422 errors on invalid input — wrong types, constraint violations like `page_size > 50` — come free with no custom error handling code.
